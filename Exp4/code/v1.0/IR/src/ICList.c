...
void writeInterCode(FILE* f, struct InterCode* head){
    for (struct InterCode* ptr = head; ptr != NULL; ptr = ptr->next){
        ...
        switch(ptr->kind){
        case ASSIGN:
            fprintf(f, "t%d := ", ptr->result.vari_no);
            switch(ptr->arg1.type){
            case 0: fprintf(f, "t%d", ptr->arg1.val); break;
            case 1: fprintf(f, "#%d", ptr->arg1.val); break;
            case 2: fprintf(f, "&t%d", ptr->arg1.val); break;
            case 3: fprintf(f, "*t%d", ptr->arg1.val);
            }
            break;
        ...
        case OPERATION:
            ...
            fprintf(f, " %s ", ptr->op.type);
            switch(ptr->arg2.type){
            case 0: fprintf(f, "t%d", ptr->arg2.val); break;
            case 1: fprintf(f, "#%d", ptr->arg2.val); break;
            case 2: fprintf(f, "&t%d", ptr->arg2.val); break;
            case 3: fprintf(f, "*t%d", ptr->arg2.val);
            }
            break;

        case LABEL:
            fprintf(f, "LABEL label%d :", ptr->result.vari_no);
            break;

        case _RETURN:
            if (ptr->arg1.type == 0)
                fprintf(f, "RETURN t%d", ptr->arg1.val);
            else if (ptr->arg1.type == 1)
                fprintf(f, "RETURN #%d", ptr->arg1.val);
            break;

        case GOTO:
            fprintf(f, "GOTO label%d", ptr->result.vari_no);
            break;

        case _RELOP:
            if (ptr->arg1.type == 0)
                fprintf(f, "IF t%d ", ptr->arg1.val);
            else if (ptr->arg1.type == 1)
                fprintf(f, "IF #%d ", ptr->arg1.val);   
            switch(ptr->relop_no){
            ...
            if (ptr->arg2.type == 0)
                fprintf(f, " t%d ", ptr->arg2.val);
            else if (ptr->arg2.type == 1)
                fprintf(f, " #%d ", ptr->arg2.val);
            fprintf(f, "GOTO label%d", ptr->result.vari_no);
            break;
        ...
        case READ:
            fprintf(f, "READ t%d", ptr->result.vari_no);
            break;
            
        case WRITE:
            if (ptr->arg1.type == 0)
                fprintf(f, "WRITE t%d", ptr->arg1.val);
            else if (ptr->arg1.type == 1)
                fprintf(f, "WRITE #%d", ptr->arg1.val);
            else if (ptr->arg1.type == 2)
                fprintf(f, "WRITE &t%d", ptr->arg1.val);
            break;
        ...
        case ARG:
            if (ptr->arg1.type == 0)
                fprintf(f, "ARG t%d", ptr->arg1.val);
            else if (ptr->arg1.type == 1)
                fprintf(f, "ARG #%d", ptr->arg1.val);
            else if (ptr->arg1.type == 2)
                fprintf(f, "ARG &t%d", ptr->arg1.val);
            break;
        
        case PARAM:
            fprintf(f, "PARAM t%d", ptr->result.vari_no);
            break;
           
        case WRITEMEM:
            if (ptr->arg1.type == 0)
                fprintf(f, "*t%d := t%d", ptr->result.vari_no, ptr->arg1.val);
            else if (ptr->arg1.type == 1)
                fprintf(f, "*t%d := #%d", ptr->result.vari_no, ptr->arg1.val);  
            break;
           
        case DEC:
            fprintf(f, "DEC t%d %d", ptr->result.vari_no, ptr->arg1.val);
            break;

        case FUNC:
            fprintf(f, "FUNCTION %s :", ptr->func_name);
            break;

        default:
            fprintf(f, "Undefined Intermediate Code Kind!");
        }
        fprintf(f, "\n");
    }
}
