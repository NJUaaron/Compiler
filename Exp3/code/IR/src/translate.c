...
struct InterCode* translate_Exp(struct Node* exp, int place){
    ...
    if (exp->child->sibling == NULL){
        ...
        if (!strcmp(exp->child->sibling->name, "ASSIGNOP")){
            ...
            //left array
                ...
                if (!strcmp(exp2->child->name, "INT")){
                    part1 = c_operation(t1, 1, exp2->child->int_val, "*", 1, width);
                }
                ...
                if (!strcmp(exp1->child->name, "ID")&&exp1->child->sibling == NULL){
                    ...
                    if (vari->inParam == 1)
                        ...
                        part2 = c_operation(t2, 3, vari->no, "+", 0, t1);
                }
                else{
                    int t3 = new_temp();
                ...
                if (!strcmp(exp3->child->name, "INT"))
                    part3 = c_writemem(t2, 1, exp3->child->int_val);
                else if (!strcmp(exp3->child->name, "ID")&&exp3->child->sibling == NULL){
            ...
            //left struct
                ...
                if (!strcmp(exp1->child->name, "ID")){
                    ...
                    if (offset == 0){
                        ...
                        if (vari->inParam == 1)
                            ...
                            part1 = c_operation(t1, 2 ,vari->no, "+", 1, offset);
                    }
                }
                else{
                ...
                if (!strcmp(exp2->child->name, "INT"))
                    part2 = c_writemem(t1, 1, exp2->child->int_val);
        ...
        //四则运算
        else if (!strcmp(exp->child->sibling->name, "PLUS")
            ||!strcmp(exp->child->sibling->name, "MINUS")
            ||!strcmp(exp->child->sibling->name, "STAR")
            ||!strcmp(exp->child->sibling->name, "DIV"))
        {
            ...
            if (!strcmp(exp->child->sibling->name, "PLUS"))
                op = "+";
            else if(!strcmp(exp->child->sibling->name, "MINUS"))
                op = "-";
            else if(!strcmp(exp->child->sibling->name, "STAR"))
                op = "*";
            else
                op = "/";
            ...
            if (!strcmp(exp1->child->name, "INT")){
                if (!strcmp(exp2->child->name, "INT"))
                    return c_operation(place, 1, exp1->child->int_val, op, 1, exp2->child->int_val);
                else if (!strcmp(exp2->child->name, "ID")){
                    struct ExtSListNode* vari = InExtList(exp2->child->id_name);
                    return c_operation(place, 1, exp1->child->int_val, op, 0, vari->no);
                }
                else{
                    int t1 = new_temp();
                    struct InterCode* code1 = translate_Exp(exp2, t1);
                    struct InterCode* code2 = c_operation(place, 1, exp1->child->int_val, op, 0, t1);
                    return bindCode(code1, code2);
                }
            }
        ...
        //MINUS
            ...
            if (!strcmp(exp1->child->name, "INT"))
                ...
                struct InterCode* code2 = c_operation(place, 1, 0, "-", 0, t1);
                return bindCode(code1, code2);
            }
        }
        //Condition
        ...
        //exp -> exp1 LB exp2 RB
            ...
            if (!strcmp(exp1->child->name, "ID")&&exp1->child->sibling == NULL){
                ...
                if (vari->inParam == 1)
                    ...
                    part2 = c_operation(place, 3, vari->no, "+", 0, t1);
            }
            else{
                int t3 = new_temp();
                struct InterCode* code1 = translate_addr(exp1, t3);    //exp1's addr
    ...
    //struct
        ...
        if (!strcmp(exp1->child->name, "ID")){
            ...
            if (vari->inParam == 1)
                ...
                return c_operation(place, 2 ,vari->no, "+", 1, offset);
        }
        else{
            int t1 = new_temp();
            struct InterCode* code1 = translate_addr(exp1, t1);    //exp1's addr
